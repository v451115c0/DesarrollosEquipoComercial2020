...
class ReconocimientosController extends Controller{
    ...
    public function MyAlcacniaDataDetail(Request $request){
        ...
        $conexion = \DB::connection('sqlsrv5');
            $genealogy = $conexion->select("SELECT a.DocSale, a.ItemCode,b.ItemName, a.Quantity, a.UnitPrice, a.U_Puntos, a.U_vol_calc FROM [dbo].[DwnPOrderLines] a LEFT JOIN LAT_MyNIKKEN.[dbo].[Catalogo_Productos] b ON ltrim(rtrim(a.itemCode)) COLLATE SQL_Latin1_General_CP850_CI_AS=ltrim(rtrim(b.itemCode)) WHERE a.DocSale= $ordernum");
        \DB::disconnect('sqlsrv5');
        $data = [
			'data' => $genealogy,
		];
